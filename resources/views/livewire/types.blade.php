...
<div class="mr-2">
       ...
       <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
           <tr>
               ...
               <th scope="col" class="px-6 py-3">
                <button class="font-semibold text-base " wire:click="sortBy('name')">  Type Name</button>
    @if($sortBy=='name')
         ...
         <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
           ...
           {{ $type->name }}
            ...
            <td class="px-6 py-4 text-right">
                <x-jet-button wire:click="typeEditModal({{ $type->id }})" class="bg-orange-500 hover:bg-orange-700">
                    {{ __('Edit') }}
                </x-jet-button>
                
                <x-jet-danger-button wire:click="typedeletemodal({{ $type->id }})" wire:loading.attr="disabled">
                    {{ __('Delete ') }}
                </x-jet-danger-button>
   ...
   <div class="mt-4">{{ $types->links() }} </div>
       ...
       <x-jet-confirmation-modal wire:model="typeconfimationmodal">
        <x-slot name="title">
            {{ __('Delete Type') }}
        </x-slot>
        ...
        <x-slot name="footer">
          ...
          <x-jet-dialog-modal wire:model="addingtypemodal">
            ...
            <x-slot name="title">
                {{ isset($this->type->id)? 'Edit Type':'Add Type'}}
            </x-slot>
            ...
            <x-slot name="content">
                ...
                <div class="col-span-6 sm:col-span-4">
                    ...
                    <x-jet-input id="name" type="text" class="mt-1 block w-full" wire:model.defer="type.name" autocomplete="name" />
                    <x-jet-input-error for="type.name" class="mt-2" />
                </div>

                <div class="col-span-6 sm:col-span-4 mt-4">
                    <x-jet-label for="description" value="{{ __('Description') }}" />
                    <x-jet-input id="description" type="text" class="mt-1 block w-full" wire:model.defer="type.description"  />
                    <x-jet-input-error for="type.description" class="mt-2" />
                </div>

                <div class="col-span-6 sm:col-span-4 mt-4">
                   
                    <x-jet-input id="status" type="checkbox" class="form-checkbox" wire:model.defer="type.status"  />
