    ...
    <div class="mr-2">
           ...
           <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
               <tr>
                   ...
                   <th scope="col" class="px-6 py-3">
                    <button class="font-semibold text-base " wire:click="sortBy('name')">  Scheme Name</button>
        @if($sortBy=='name')
           ...
           <tbody>
             ...
             <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
               ...
               {{ $scheme->name }}
                ...
                <td class="px-6 py-4">
                   {{ $scheme->status? 'Active':'Not-Active'  }}
                </td>
                <td class="px-6 py-4">
                   {{ $scheme->user->name }}
                </td>
                <td class="px-6 py-4 text-right">
                    <x-jet-button wire:click="typeEditModal({{ $scheme->id }})" class="bg-orange-500 hover:bg-orange-700">
                        {{ __('Edit') }}
                    </x-jet-button>
                    
                    <x-jet-danger-button wire:click="typedeletemodal({{ $scheme->id }})" wire:loading.attr="disabled">
                        {{ __('Delete ') }}
                    </x-jet-danger-button>
       ...
       <div class="mt-4">{{ $schemes->links() }} </div>
           ...
           <x-jet-dialog-modal wire:model="typeconfimationmodal">
            ...
            <x-slot name="content">
                {{ __('Are you sure you want to delete this scheme? ') }}
            ...
            <x-slot name="footer">
              ...
              <x-jet-dialog-modal wire:model="addingtypemodal">
                ...
                <x-slot name="title">
                    {{ isset($this->scheme->id)? 'Edit Scheme':'Add Scheme'}}
                </x-slot>
                ...
                <x-slot name="content">
                    ...
                    <div class="col-span-6 sm:col-span-4">
                        ...
                        <x-jet-input id="name" type="text" class="mt-1 block w-full" wire:model.defer="scheme.name" autocomplete="name" />
                        <x-jet-input-error for="scheme.name" class="mt-2" />
                    </div>

                    <div class="col-span-6 sm:col-span-4">
                        <x-jet-label for="pashto_name" value="{{ __('Pashto Name') }}" />
                        <x-jet-input id="pashto_name" type="text" class="mt-1 block w-full" wire:model.defer="scheme.pashto_name" autocomplete="name" />
                        <x-jet-input-error for="scheme.pashto_name" class="mt-2" />
                    </div>
    
                    <div class="col-span-6 sm:col-span-4 mt-4">
                        <x-jet-label for="description" value="{{ __('Description') }}" />
                        <x-jet-input id="description" type="text" class="mt-1 block w-full" wire:model.defer="scheme.description"  />
                        <x-jet-input-error for="scheme.description" class="mt-2" />
                    </div>
    
                    <div class="col-span-6 sm:col-span-4 mt-4">
                       
                        <x-jet-input id="status" type="checkbox" class="form-checkbox" wire:model.defer="scheme.status"  />
