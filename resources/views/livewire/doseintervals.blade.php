...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    ...
    <div class="mr-2">
           ...
           <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
               <tr>
                   ...
                   <th scope="col" class="px-6 py-3">
                    <button class="font-semibold text-base " wire:click="sortBy('name')">  Dose Interval Name</button>
        @if($sortBy=='name')
           ...
           <tbody>
             ...
             <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
               ...
               {{ $doseinterval->name }}
                ...
                <th scope="row" class="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                    {{ $doseinterval->pashto_name }}
                     </th>
                <td class="px-6 py-4">
                   {{ $doseinterval->description }}
                </td>
                <td class="px-6 py-4">
                   {{ $doseinterval->status? 'Active':'Not-Active'  }}
                </td>
                <td class="px-6 py-4">
                   {{ $doseinterval->user->name }}
                </td>
                <td class="px-6 py-4 text-right">
                    <x-jet-button wire:click="typeEditModal({{ $doseinterval->id }})" class="bg-orange-500 hover:bg-orange-700">
                        {{ __('Edit') }}
                    </x-jet-button>
                    
                    <x-jet-danger-button wire:click="typedeletemodal({{ $doseinterval->id }})" wire:loading.attr="disabled">
                        {{ __('Delete ') }}
                    </x-jet-danger-button>
       ...
       <div class="mt-4">{{ $doseintervals->links() }} </div>
           ...
           <x-jet-dialog-modal wire:model="typeconfimationmodal">
            ...
            <x-slot name="content">
                {{ __('Are you sure you want to delete this dose interval? ') }}
            ...
            <x-slot name="footer">
              ...
              <x-jet-dialog-modal wire:model="addingtypemodal">
                ...
                <x-slot name="title">
                    {{ isset($this->doseinterval->id)? 'Edit Dose Interval':'Add Dose Interval'}}
                </x-slot>
                ...
                <x-slot name="content">
                    ...
                    <div class="col-span-6 sm:col-span-4">
                        ...
                        <x-jet-input id="name" type="text" class="mt-1 block w-full" wire:model.defer="doseinterval.name" autocomplete="name" />
                        <x-jet-input-error for="doseinterval.name" class="mt-2" />
                    </div>

                    <div class="col-span-6 sm:col-span-4">
                        <x-jet-label for="pashto_name" value="{{ __('Pashto Name') }}" />
                        <x-jet-input id="pashto_name" type="text" class="mt-1 block w-full" wire:model.defer="doseinterval.pashto_name" autocomplete="name" />
                        <x-jet-input-error for="doseinterval.pashto_name" class="mt-2" />
                    </div>
    
                    <div class="col-span-6 sm:col-span-4 mt-4">
                        <x-jet-label for="description" value="{{ __('Description') }}" />
                        <x-jet-input id="description" type="text" class="mt-1 block w-full" wire:model.defer="doseinterval.description"  />
                        <x-jet-input-error for="doseinterval.description" class="mt-2" />
                    </div>
    
                    <div class="col-span-6 sm:col-span-4 mt-4">
                       
                        <x-jet-input id="status" type="checkbox" class="form-checkbox" wire:model.defer="doseinterval.status"  />
